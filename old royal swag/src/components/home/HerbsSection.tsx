...
export function HerbsSection() {
  ...
  return (
    <section
      id="herbs"
      className="flex flex-col gap-6 overflow-hidden py-10 pl-5 md:mx-auto md:max-w-6xl md:px-16"
    >
      <div className="flex items-end justify-between pr-5">
        <div>
          <h2 className="font-display text-[32px] font-bold text-[#324023]">
            The Sacred Seven
          </h2>
      ...
      <div
        className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-4 pr-5"
        style={{ scrollbarWidth: "none" }}
      >
        {herbs.map((herb) => (
          <div
            key={herb.name}
            className="w-56 shrink-0 snap-center cursor-pointer overflow-hidden rounded-2xl md:w-64"
            style={{ height: "320px" }}
            onMouseEnter={() => setHoveredHerb(herb.name)}
            onMouseLeave={() => setHoveredHerb(null)}
          >
            <div
              className="relative h-full w-full transition-all duration-500 ease-out"
              style={{
                transform: hoveredHerb === herb.name ? "scale(1.02)" : "scale(1)",
              }}
            >
              <div
              ...
              >
                ...
                <img
                  ...
                  style={{
                    transform:
                      hoveredHerb === herb.name ? "scale(1.1)" : "scale(1)",
                  }}
                  onError={(e) => {
                ...
                style={{
                  transform:
                    hoveredHerb === herb.name
                      ? "translateY(0)"
                      : "translateY(100%)",
                  opacity: hoveredHerb === herb.name ? 1 : 0,
                }}
              >
